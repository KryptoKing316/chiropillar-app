...
export async function POST(req: Request) {
  try {
    const { cim_data } = await req.json()
    if (!cim_data) return NextResponse.json({ error: 'No CIM data provided' }, { status: 400 })

    const html = buildCIMHtml(cim_data)

    // Return HTML for client-side print-to-PDF
    // (Puppeteer not available in Vercel edge; browser print is more reliable)
    ...
    console.error('CIM export error:', err)
    return NextResponse.json({ error: 'Export failed' }, { status: 500 })
  }
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function buildCIMHtml(cim: Record<string, any>): string {
  ...
  const growth: Record<string, string>[] = cim.growth_opportunities || []
  const ds = cim.deal_structure || {}
  const ih: string[] = cim.investment_highlights || []
...
<head>
<meta charset="utf-8"/>
<title>${es.headline || 'CIM'} — Kingdom Broker</title>
<link rel="preconnect" href="https://fonts.googleapis.com"/>
<link href="https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;600;700&family=DM+Sans:wght@400;500;600;700&display=swap" rel="stylesheet"/>
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: 'DM Sans', sans-serif; background: #fff; color: #1a1a2e; font-size: 11pt; line-height: 1.65; }
...
<body>
<div class="no-print" style="background:#0B1B3E;color:#C9A84C;text-align:center;padding:12px;font-family:'DM Sans',sans-serif;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;font-size:11pt;cursor:pointer;" onclick="window.print()">
  ⬇ Click here to Print / Save as PDF — then select "Save as PDF" from the print dialog
</div>
<div class="page">
  <div class="confidential-header">
    <span style="font-size:9pt;font-weight:700;color:#C9A84C;letter-spacing:0.15em;text-transform:uppercase;">Confidential Information Memorandum</span>
  ...
  <div class="section">
    ...
    ${ih.map(h => `<div class="ih-item"><span style="color:#C9A84C;flex-shrink:0;">◆</span><span>${h}</span></div>`).join('')}
  </div>` : ''}
  ...
  <div class="section">
    ...
    <table>
      ...
      <tbody>
        ${fs.income_statement.map((r: Record<string, string>) => `
          <tr>
            <td><strong style="color:#C9A84C;">${r.year || ''}</strong></td>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:4px;">
      ...
      ${fs.valuation ? `
      <div style="background:#fff8e7;border:1px solid #C9A84C;border-radius:8px;padding:10px 14px;">
  ...
  <div class="section">
    ...
    ${ma.barriers_to_entry.map((b: string) => `
      <div class="ih-item"><span style="color:#C9A84C;flex-shrink:0;">◆</span><span style="font-size:9.5pt;color:#555;">${b}</span></div>
    `).join('')}` : ''}
  </div>` : ''}
